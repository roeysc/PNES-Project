function create_lab_quickcap64_mcn()
...
indices = [];
for i = 1:length(lab_quickcap64_mcn.label)
    switch lab_quickcap64_mcn.label{i}
        case 'FT7'
            indices(end+1) = i;
        case 'FT8'
            indices(end+1) = i;
...
lab_quickcap64_mcn.pos(indices,:) = [];
lab_quickcap64_mcn.width(indices,:) = [];
lab_quickcap64_mcn.height(indices,:) = [];
lab_quickcap64_mcn.label(indices,:) = [];

lay = lab_quickcap64_mcn; % all layouts in fieldtrip are called 'lay'


% Save and plot the layout for everyone to see 
save(fullfile(fileparts(fullfile(which('quickcap64.mat'))), 'lab_quickcap64_mcn'), 'lay');

cfg = [];
cfg.layout = fullfile(fileparts(fullfile(which('quickcap64.mat'))), 'lab_quickcap64_mcn');
