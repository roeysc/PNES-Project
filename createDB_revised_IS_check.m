...
% cfg.validEegLabels = 'C:\Users\Roey\Documents\MATLAB\fieldtrip-20140720\template\layout\lab_quickcap64_mcn_no_cb.mat';
% createDB_revised(cfg, 'C:\Users\Roey\Documents\Lab\PNES Project\_forISCheckRoeyEEG\Roey')
...
for i=1:length(subDirs)
    ...
    for j=1:length(recordingFilesInSubDir)
        ...
        eegTime = load(fullfile( inputDir, subDirs(i).name, [recordingFilesInSubDir(j).name, '.TIME.mat'] )); % eegTime defines the absolute time (date, hour etc, of the recording)
        ...
        segmentsCell = spiltByMarker(eeg, segmentsCell, 'RS|MHRS|MHLS|MLRS|MLLS|SHRS|SHLS|SARS|SALS|SLRS|SLLS|SMS|MUSICS|VISUALS');
        ...
        saveSegments(segmentsCell, inputDir, subDirs(i).name, eeg.chan, eeg.srate)
...
function segmentsCell = spiltByMarker(eeg, segmentsCell, validMarkersStart)
% spiltByMarker splits the signal in eeg.sig by the the valid markers in eeg.mrk
...
for markerI = 1:length(eeg.mrk{markerNameColumn})
    ...
    prefix = [];
    infix = [];
    if(~all(isempty(regexpi(eeg.mrk{markerNameColumn}{markerI},['^(' validMarkersStart ')_(U|O|C)_[\d]*[*]*$']))))
        disp('Marker found!');
        eeg.mrk{markerNameColumn}{markerI}
...
function saveSegments(segmentsCell, inputDir, subDirName, validEegChannels, srate)
...
for segmentI = 1:length(segmentsCell)
    if( flags(segmentI) == 1)
        ...
        segmentsTypeName = cell2mat([segmentsCell{segmentI}{2},'_', segmentsCell{segmentI}{3}, '_', segmentsCell{segmentI}{4}]);
