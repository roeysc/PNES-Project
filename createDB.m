...
% For each patient
   for i=1:length(subDirs)
    ...
    filesInSubDir = dir([inputDir subDirs(i).name '\*.TRC.mat']); % TODO: HERE WE MUST CHANGE
    segmentsCell = {};
    ...
    for j=1:length(filesInSubDir)
        ...
        filesInSubDir(j).name = filesInSubDir(j).name(1:end-4);
...
eeg.sig = eeg.sig(validChansLocVec,:);
        ...
        for markerI = 1:length(eeg.mrk{markerNameColumn})
            ...
            prefix = [];
            infix = [];
            if(~all(isempty(regexpi(eeg.mrk{markerNameColumn}{markerI},'^(RS|RTS|BPS|PS|ES|BES)_(UN|ON|CN)_[\d]*[*]*$'))))
                disp('Marker found!');
                eeg.mrk{markerNameColumn}{markerI}
                ...
                markerTypeS = regexpi(eeg.mrk{markerNameColumn}{markerI},'(RS|RTS|BPS|PS|ES|BES)','match'); 
                markerTypeE = [markerTypeS{1}(1:end-1) 'E'];
                
                for coMarkerI = markerI+1:length(eeg.mrk{markerNameColumn})
                    if(~all(isempty(regexpi(eeg.mrk{markerNameColumn}{coMarkerI},cell2mat(['^' markerTypeE '_(UN|ON|CN)_' numberSTR starSTR '$'])))))
